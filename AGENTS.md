...
### Page file convention (traditional web)
...
- Bootstrap (`scripts/bootstrap.ts`) imports `main.css` + Alpine + plugins + fonts + service worker
...
- **webpackPreload magic comments**: `bootstrap.ts` dynamic imports use `/* webpackPreload: true */` for critical chunks such as alpine, i18next, and i18n-formatters.
...
- **Single-locale i18n optimization**: stub Alpine store, skip i18next init when only 1 locale
...
- **Destructive git operations** (`reset --hard`, `rebase`) require `git status` first
