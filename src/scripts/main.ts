...
} from "@/configs/locales/writing-systems";
import { preloadActiveFont } from "./lib/font-preloader";
import { watchScriptAndLoadFont } from "./lib/font-watcher";
import {
  getLanguageConfig,
  getLocale,
...
> = {
  ...
  [NUMBERING_SYSTEM_CODE.CYRL]: createPrimaryLoader(
    WRITING_SYSTEM_CODE.CYRILLIC,
  ),
  [NUMBERING_SYSTEM_CODE.HEBR]: createPrimaryLoader(WRITING_SYSTEM_CODE.HEBREW),
  [NUMBERING_SYSTEM_CODE.GREK]: createPrimaryLoader(WRITING_SYSTEM_CODE.GREEK),
...
export const setupFontStackCSS = (): void => {
  ...
  const nsCode = locale?.nativeNumberingSystem as
    | NumberingSystemCode
    | undefined;
  const ns = nsCode
    ? NUMBERING_SYSTEMS.find((n) => n.code === nsCode)
    : undefined;

  const primaryFont = ns?.fontFamily ?? FONT_STACK.primary.family;
