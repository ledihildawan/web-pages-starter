...
function getTsconfigTarget() {
  ...
  try {
    ...
    tsconfigTargetCache = tsconfig.compilerOptions?.target || 'es2020';
  } catch {
...
export async function preprocessLoadTags(source, contextDir, options = {}) {
  const loadTemplates = [];
  ...
  while ((match = regex.exec(source)) !== null) {
    ...
    if (hasInlineLoaders(templatePath)) {
      ...
      if (inlineMatch) {
        ...
        const resolvedPath = contextDir ? path.resolve(contextDir, filePath) : filePath;

        if (loaderChain === 'raw') {
          try {
            const content = fs.readFileSync(resolvedPath, 'utf8');
            const id = `__inline_raw_${inlineLoaderId++}__`;
            inlineRawContent.push({ id, content });
        ...
        if (loaderChain === 'scss') {
          try {
            const sassResult = sass.compile(resolvedPath);
            const id = `__inline_scss_${inlineLoaderId++}__`;
            inlineRawContent.push({ id, content: sassResult.css });
            inlineFilePaths.push(resolvedPath);
            cleanedSource = cleanedSource.replace(
              fullMatch,
        ...
        if (loaderChain === 'sass') {
          try {
            const sassResult = sass.compile(resolvedPath, { indentedSyntax: true });
        ...
        if (loaderChain === 'less') {
          try {
            const lessResult = await less.render(fs.readFileSync(resolvedPath, 'utf8'), { filename: resolvedPath });
            const id = `__inline_less_${inlineLoaderId++}__`;
            inlineRawContent.push({ id, content: lessResult.css });
            inlineFilePaths.push(resolvedPath);
            cleanedSource = cleanedSource.replace(
              fullMatch,
        ...
        if (loaderChain === 'js') {
          try {
            ...
            inlineRawContent.push({ id, content: bundledCode });
            inlineFilePaths.push(resolvedPath);
            cleanedSource = cleanedSource.replace(
              fullMatch,
        ...
        if (loaderChain === 'json') {
          try {
            ...
            inlineRawContent.push({ id, content: JSON.stringify(jsonContent) });
            inlineFilePaths.push(resolvedPath);
            cleanedSource = cleanedSource.replace(
              fullMatch,
        ...
        if (loaderChain === 'yaml') {
          try {
            ...
            inlineRawContent.push({ id, content: JSON.stringify(yamlContent) });
            inlineFilePaths.push(resolvedPath);
            cleanedSource = cleanedSource.replace(
              fullMatch,
        ...
        if (loaderChain === 'json5') {
          try {
            const content = fs.readFileSync(resolvedPath, 'utf8');
            const json5Content = json5.default ? json5.default.parse(content) : json5.parse(content);
            const id = `__inline_json5_${inlineLoaderId++}__`;
            inlineRawContent.push({ id, content: JSON.stringify(json5Content) });
            inlineFilePaths.push(resolvedPath);
            cleanedSource = cleanedSource.replace(
              fullMatch,
        ...
        if (loaderChain === 'md' || loaderChain === 'markdown') {
          try {
            ...
            inlineRawContent.push({ id, content: htmlContent });
            inlineFilePaths.push(resolvedPath);
            cleanedSource = cleanedSource.replace(
              fullMatch,
        ...
        if (loaderChain === 'toml') {
          try {
            ...
            inlineRawContent.push({ id, content: JSON.stringify(tomlContent) });
            inlineFilePaths.push(resolvedPath);
            cleanedSource = cleanedSource.replace(
              fullMatch,
        ...
        if (loaderChain === 'xml') {
          try {
            ...
            inlineRawContent.push({ id, content: JSON.stringify(xmlContent) });
            inlineFilePaths.push(resolvedPath);
            cleanedSource = cleanedSource.replace(
              fullMatch,
        ...
        if (loaderChain === 'base64') {
          try {
            ...
            inlineRawContent.push({ id, content: base64Content });
            inlineFilePaths.push(resolvedPath);
            cleanedSource = cleanedSource.replace(
              fullMatch,
        ...
        if (loaderChain === 'url') {
          try {
            ...
            inlineRawContent.push({ id, content: dataUri });
            inlineFilePaths.push(resolvedPath);
            cleanedSource = cleanedSource.replace(
              fullMatch,
        ...
        if (loaderChain === 'glob') {
          try {
            ...
            inlineRawContent.push({ id, content: JSON.stringify(contents) });
        ...
        if (loaderChain === 'csv') {
          try {
            ...
            inlineRawContent.push({ id, content: JSON.stringify(csvData) });
            inlineFilePaths.push(resolvedPath);
            cleanedSource = cleanedSource.replace(
              fullMatch,
        ...
        if (loaderChain === 'ini') {
          try {
            ...
            inlineRawContent.push({ id, content: JSON.stringify(iniData) });
            inlineFilePaths.push(resolvedPath);
            cleanedSource = cleanedSource.replace(
              fullMatch,
    ...
    if (isAbsoluteTemplatePath(templatePath)) {
      ...
      const resolvedPath = path.resolve(contextDir, templatePath);
  ...
  return { cleanedSource, loadTemplates, inlineRawContent, inlineFilePaths };
}
