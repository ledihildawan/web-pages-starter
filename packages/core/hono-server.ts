...
export function createStaticApp(distDir: string, htmlCache?: Map<string, string>): Hono {
  ...
  app.use('/*', async (c, next) => {
    ...
    const isHtmlRequest = path === '/' || HTML_RE.test(path);
    if (isHtmlRequest) {
      return next();
    }
    return serveStatic({ root: './dist', rewriteRequestPath: (p) => p, precompressed: true })(c, next);
  });
  ...
  app.use('*', async (c, next) => {
    ...
    const isCameraPage = reqPath.includes('test-camera');
    c.res.headers.set(
      'Permissions-Policy',
      isCameraPage ? 'camera=(self), microphone=(), geolocation=()' : 'camera=(), microphone=(), geolocation=()',
    );
  });
  ...
  app.get('*', (c) => {
    ...
    const processedHtml = html ? injectNonce(html, nonce) : '';
