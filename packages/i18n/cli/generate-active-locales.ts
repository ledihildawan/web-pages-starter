...
const activeLanguageCodes: string[] = [...new Set(activeLocales.map((l) => l.language))];
const activeNsCodes: string[] = [
  ...new Set(activeLocales.flatMap((l) => [l.numberingSystem, l.nativeNumberingSystem])),
];
const activeWsCodes: string[] = [...new Set(activeLocales.map((l) => l.writingSystem))];

const languageEntries = LANGUAGES.filter((l) => activeLanguageCodes.includes(l.code));
const nsEntries = NUMBERING_SYSTEMS.filter((s) => activeNsCodes.includes(s.code));
const wsEntries = WRITING_SYSTEMS.filter((ws) => activeWsCodes.includes(ws.code));
