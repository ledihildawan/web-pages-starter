...
- `public/service-worker.js` — **generated** by `scripts/generators/generate-service-worker.ts` (not static). Stale-while-revalidate service worker with navigation cache and asset caching. Derives `BASE_PATH` from `self.location`, precaches `index.html`, 6 error pages, `manifest.json`, `favicon.svg`. Offline fallback to `offline.html`. Cache version: dynamic — `starter-<base36 timestamp>`, regenerated each build
