...
async function bootstrap() {
  try {
    ...
    const [storeModule, navbarModule, runtimeModule] = await Promise.all([
      import(/* webpackPreload: true */ '@i18n/runtime/store').then((m) => m.default),
      import(/* webpackPreload: true */ '@/shared/ui/composites/navbar').then((m) => m.default),
      import(/* webpackPreload: true */ '@i18n/runtime/runtime').then((m) => ({
        i18next: m.i18next,
        initIntl: m.initIntl,
      })),
    ]);

    for (const state of [storeModule, navbarModule]) {
      if (state.type === 'data') {
    ...
    if (!runtimeModule.i18next.isInitialized) {
      await runtimeModule.initIntl(savedLocale);
    }
...
window.addEventListener('error', (e) => console.error('ERROR:', e.message, e.filename));
