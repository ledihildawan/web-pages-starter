export const log = {
  ...
  header: (title: string) => {
    ...
    console.log(`┌${line}┐`);
    console.log(`│${title.padStart(Math.floor((42 + title.length) / 2)).padEnd(42)}│`);
    console.log(`└${line}┘\n`);
  },
  ...
  serverPortError: (port: number) => {
    ...
    console.error('  Stop the other process or set PORT in .env to a different value.\n');
  },
  serverError: (err: NodeJS.ErrnoException) => {
    console.error(`\nError: Server failed — ${err}`);
  },
  distNotFound: () => console.log('dist/ not found — run `bun run build` first.'),
};

const BOX_WIDTH = 40;

export function logBox(title: string, entries: Record<string, string | number>): void {
  const inner = BOX_WIDTH;
  log.info(`┌${'─'.repeat(inner + 2)}┐`);
  log.info(`│ ${title.padStart(Math.floor((inner + title.length) / 2)).padEnd(inner)} │`);
  log.info(`├${'─'.repeat(inner + 2)}┤`);
