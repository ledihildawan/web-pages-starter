export class VideoStoryPlayer {
  constructor(options = {}) {
    ...
    this.format =
      this.triggerGroup.getAttribute("data-story-format") || "default";
    this.triggers = Array.from(
      this.triggerGroup.querySelectorAll("[data-story-trigger]"),
    );
    this.currentTriggerIndex = options.currentTriggerIndex || 0;
    this.activeTrigger = this.triggers[this.currentTriggerIndex];
    this.items = Array.from(
      this.activeTrigger.querySelectorAll("[data-story-item]"),
    );
    ...
    this.data = {
      ...
      readMoreText:
        this.currentItem.getAttribute("data-story-readmore") || "Selengkapnya",
      altText: this.currentItem.getAttribute("data-story-alt") || "Story Media",
      background:
        this.currentItem.getAttribute("data-story-background") || "#000000",
  ...
  init() {
    document
      .querySelectorAll(".video-player-container")
  ...
  getVideoElement() {
    ...
    const mediaStyle = `style="background-color: transparent; position: relative; z-index: 1;"`;

    if (type === "youtube") {
      const youtubeId =
        videoUrl.includes("youtube") || videoUrl.includes("youtu.be")
          ? this.getYouTubeId(videoUrl)
          : videoUrl;
      return `<iframe id="mediaPlayer" loading="lazy" class="unified-media-element" ${mediaStyle} src="https://www.youtube.com/embed/${youtubeId}?autoplay=1&controls=0&rel=0&showinfo=0&enablejsapi=1" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen title="${altText}"></iframe>`;
    } else if (type === "mp4") {
      return `<video class="unified-media-element" id="mediaPlayer" autoplay playsinline preload="auto" title="${altText}" ${mediaStyle}>
                        <source src="${videoUrl}" type="video/mp4">
                    </video>`;
    } else if (type === "image") {
      return `<img class="unified-media-element img-element" id="mediaPlayer" src="${videoUrl}" alt="${altText}" ${mediaStyle}>`;
    } else {
      return `<iframe id="mediaPlayer" loading="lazy" class="unified-media-element" src="${videoUrl}" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen title="${altText}" ${mediaStyle}></iframe>`;
    }
  }
  ...
  getTemplate() {
    ...
    // --- LOGIKA BACKGROUND DIV ---
    const background = this.data.background;
    let bgStyleString = "background-color: #000000;"; // Default hitam

    if (background) {
      const isImage =
        /\.(jpeg|jpg|gif|png|webp|svg)$/i.test(background) ||
        background.includes("/");
      if (isImage) {
        const bgImage = background.includes("url(")
          ? background
          : `url('${background}')`;
        bgStyleString = `background: ${bgImage} center/cover no-repeat;`;
      } else {
        bgStyleString = `background-color: ${background};`;
      }
    }
    // ------------------------------
    ...
    return `
            <div class="video-player-container">
                <div class="unified-video-wrapper">
                    ...
                    <div class="story-background" style="position: absolute; inset: 0; z-index: 0; ${bgStyleString}"></div>
  ...
  cacheDOM() {
    ...
    if (this.format === "webstory") {
      ...
      this.shareBtnDesktop = this.element.querySelector(
        "#btn-share-social-desktop",
      );
      this.shareMenuDesktop = this.element.querySelector(
        "#share-link-media-desktop",
      );
      ...
      this.sectionContentDesc = this.element.querySelector(
        "#section-content-description",
      );

      this.popupShortsOverlay = this.element.querySelector(
        ".popup-shorts-overlay",
      );
      this.popupShortsContent = this.element.querySelector(
        ".popup-shorts-content",
      );
      ...
      this.popupShareOverlay = this.element.querySelector(
        ".popup-share-overlay",
      );
      this.popupShareContent = this.element.querySelector(
        ".popup-share-content",
      );
  ...
  bindEvents() {
    if (this.closeBtn)
      this.closeBtn.addEventListener("click", () => this.destroy(true));
    document.addEventListener("keydown", this.handleKeydown);

    if (this.scrollUp)
      this.scrollUp.addEventListener("click", () => this.playPrevTrigger());
    if (this.scrollDown)
      this.scrollDown.addEventListener("click", () => this.playNextTrigger());

    if (this.leftOverlay)
      this.leftOverlay.addEventListener("click", () => this.playPrevItem());
    if (this.rightOverlay)
      this.rightOverlay.addEventListener("click", () => this.playNextItem());
    ...
    if (this.centerOverlay && this.mediaPlayer) {
      const pauseMedia = () => {
        ...
        if (this.data.type === "mp4") {
          ...
          this.mediaPlayer.contentWindow.postMessage(
      ...
      this.centerOverlay.addEventListener("touchstart", pauseMedia, {
        passive: true,
      });
    ...
    if (this.playPauseBtn && this.mediaPlayer) {
      this.playPauseBtn.addEventListener("click", (e) => {
        ...
        this.playPauseIcon.src = this.isPaused
          ? "https://telkomsel.com//themes/custom/telkomsel/assets/img/revamp/play.svg"
          : "https://telkomsel.com//themes/custom/telkomsel/assets/img/revamp/pause.svg";
      });
    }
    ...
    if (this.muteBtn && this.mediaPlayer) {
      if (this.data.type === "image") {
        ...
        this.muteBtn.addEventListener("click", () => {
          ...
          this.muteIcon.src = this.mediaPlayer.muted
            ? "https://telkomsel.com//themes/custom/telkomsel/assets/img/revamp/mute.svg"
            : "https://telkomsel.com//themes/custom/telkomsel/assets/img/revamp/sound.svg";
        });
      }
    }
    ...
    if (this.format !== "webstory") {
      if (this.shareBtnDesktop && this.shareMenuDesktop) {
        this.shareBtnDesktop.addEventListener("click", () => {
          this.shareMenuDesktop.style.display =
            window.getComputedStyle(this.shareMenuDesktop).display === "none"
              ? "flex"
              : "none";
        });
      }
      ...
      if (this.emotes) {
        this.emotes.forEach((emote) => {
          emote.addEventListener("click", (e) =>
            this.animateEmote(e.currentTarget),
          );
        });
      }
      ...
      if (this.mobileVideoInfo)
        this.mobileVideoInfo.addEventListener("click", openDescModal);
      if (this.sectionContentDesc)
        this.sectionContentDesc.addEventListener("click", openDescModal);
      if (this.shareBtnMobile)
        this.shareBtnMobile.addEventListener("click", openShareModal);

      if (this.closeDescBtn)
        this.closeDescBtn.addEventListener("click", (e) => {
          e.stopPropagation();
          closeAllModals();
        });
      if (this.closeShareBtn)
        this.closeShareBtn.addEventListener("click", (e) => {
          e.stopPropagation();
          closeAllModals();
        });
    ...
    if (this.data.type === "mp4" && this.mediaPlayer) {
      this.mediaPlayer.addEventListener("timeupdate", () => {
        if (!this.isPaused && this.currentProgressFill) {
          const percentage =
            (this.mediaPlayer.currentTime / this.mediaPlayer.duration) * 100;
      ...
      const animateImageProgress = () => {
        if (!this.isPaused) {
          ...
          if (percentage >= 100) {
            if (this.currentProgressFill)
              this.currentProgressFill.style.width = "100%";
            this.playNextItem();
            return;
          } else {
            if (this.currentProgressFill)
  ...
  animateEmote(emoteElement) {
    ...
    const animation = floatingEmote.animate(
      [
        ...
        {
          transform: `translateY(-${flyDistance / 1.2}px) scale(1)`,
          opacity: 0,
        },
      ],
  ...
  static init() {
    ...
    triggers.forEach((trigger) => {
      trigger.addEventListener("click", function () {
        ...
        const triggersInGroup = Array.from(
          group.querySelectorAll("[data-story-trigger]"),
        );
    ...
    if (videoId) {
      ...
      if (item) {
        ...
        const itemsInTrigger = Array.from(
          trigger.querySelectorAll("[data-story-item]"),
        );
